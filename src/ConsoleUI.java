...
public class ConsoleUI {
    ...
    public void addHuman(String[] array) throws Exception {
        ...
        fileHandler.fileWr(fileName(firstName), human);
    }

    public void dataCheck(String[] array) throws NumberParametersException {
        if (array.length != 6)
            if (array.length < 6) {
                throw new NumberParametersException("Количество переданных параметров меньше необходимого.",
                        array.length);
            } else {
                throw new NumberParametersException("Количество переданных параметров превышает необходимое.",
                        array.length);
            }
    }
